...
async function SearchResults({ query }: { query: string }) {
  ...
  // First, try to find existing articles
  ...
  // If no articles found, automatically generate one
  if (!articles || articles.length === 0) {
    console.log(`[Search] No articles found for "${query}", generating...`);
    
    // Try to generate an article for this topic
        ...
        return (
          <>
            <div className="mb-6 p-4 bg-accent/10 border border-accent/30 rounded-lg">
              <p className="text-accent font-medium">New article automatically generated for "{query}"</p>
            </div>
            <div className="space-y-4">
              {newArticles.map((article) => (
                <Link key={article.id} href={`/article/${article.slug}`}>
                  <Card className="transition-all hover:shadow-md border-accent">
                    <CardContent className="p-6">
                      <div className="flex items-start gap-4">
                        <div className="flex-1">
                          ...
                          <span className="inline-block mt-2 text-xs bg-accent/10 text-accent px-2 py-0.5 rounded">AI Generated</span>
                        </div>
  ...
  // No articles found and couldn't generate
  return (
        ...
        {query ? (
          <Suspense fallback={
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mt-4">Searching and generating article...</p>
            </div>
          }>
